...
export async function updateCRMWhatsAppNumber(partnerId: string, whatsappMobile: string) { return readJson(await crmStaffApiFetch(`/api/cp-crm/mine/partners/${encodeURIComponent(partnerId)}/whatsapp-number`, { method: "PATCH", body: JSON.stringify({ whatsappMobile }) }), "Unable to update WhatsApp number"); }

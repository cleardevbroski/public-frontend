...
export default function AdminChannelPartners() {
  ...
  return <AdminLayout>
    ...
    <div className="admin-record-layout grid xl:grid-cols-[minmax(0,1fr)_470px] gap-5 items-start">
      ...
      <RecordDetailPanel activeId={selected?.id} onClose={() => setSelected(null)}>{detailLoading ? <div className="p-12 flex justify-center"><Loader2 className="size-7 animate-spin text-[#DDAA42]" /></div> : selected ? <PartnerDetail partner={selected} reason={reason} setReason={setReason} act={act} addNote={addNote} remove={remove} busy={busy} /> : <div className="py-14 text-center"><Eye className="size-9 mx-auto text-[#D8D4DC]" /><p className="text-[13px] text-[#68646F] mt-3">Select an application to review.</p></div>}</RecordDetailPanel>
    </div>
  </AdminLayout>;
}
...
function PartnerDetail({ partner, reason, setReason, act, addNote, remove, busy }: { partner: ChannelPartnerApplication; reason: string; setReason: (value: string) => void; act: (status: ChannelPartnerStatus) => void; addNote: () => void; remove: () => void; busy: boolean }) {
  ...
  const resendEmail = async () => { if (!partner.id) return; setEmailBusy(true); setEmailMessage(""); try { const result = await resendChannelPartnerRegistrationEmail(partner.id); setEmailMessage(result.message); } catch (cause) { setEmailMessage(cause instanceof Error ? cause.message : "Unable to resend registration email"); } finally { setEmailBusy(false); } };
  return <div><div className="flex items-start justify-between gap-3"><div><p className="text-[10.5px] uppercase font-bold tracking-wide text-[#DDAA42]">{partner.applicationNumber}</p><h2 className="text-lg font-bold text-[#121B35] mt-1">{partner.company.name}</h2><span className="inline-flex mt-1 rounded-md bg-[#F1F4FA] px-1.5 py-0.5 text-[9px] font-bold uppercase text-[#596277]">{partnerType === "individual" ? "Individual Partner" : "Company / Firm"}</span>{partner.partnerCode && <p className="mt-2 font-mono text-sm font-extrabold tracking-wider text-[#121B35]">{partner.partnerCode}</p>}</div>{partner.status && <StatusBadge status={partner.status} />}</div>
    {partner.sensitiveDataAvailable === false && <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-[11px] text-amber-800">The application loaded, but encrypted bank/code fields are unavailable. Restore the encryption key that was active when this application was submitted.</div>}
    <button disabled={emailBusy || !partner.partnerCode} onClick={() => void resendEmail()} className="mt-3 h-9 px-3 rounded-lg border border-[#E4E0E7] text-[11px] font-bold disabled:opacity-40">{emailBusy ? "Sending..." : "Resend registration email"}</button>{emailMessage && <p className="mt-2 text-[11px] text-[#68646F]">{emailMessage}</p>}
...
function PartnerClients({ partnerId }: { partnerId: string }) {
  type PartnerClient = { id: string; clientName: string; mobileMasked: string; leadNumber: string; projectTitle: string; status: string; ownershipExpiresAt: string };
  type PartnerClash = { id: string; direction: string; clientName: string; mobileMasked: string; projectTitle: string; attemptedAt: string };
  type PartnerSummary = { active: PartnerClient[]; history: PartnerClient[]; recentClashes: PartnerClash[]; counts: Record<string, number>; earnings: { totalCreditedPaise: number; awaitingInitialPaise: number } };
  const [data, setData] = useState<PartnerSummary>({ active: [], history: [], recentClashes: [], counts: {}, earnings: { totalCreditedPaise: 0, awaitingInitialPaise: 0 } });
  const [loading, setLoading] = useState(true);
  useEffect(() => { let current = true; setLoading(true); fetchAdminChannelPartnerClients(partnerId).then((result) => { if (current) setData({ active: result.active || [], history: result.history || [], recentClashes: result.recentClashes || [], counts: result.counts || {}, earnings: result.earnings || { totalCreditedPaise: 0, awaitingInitialPaise: 0 } }); }).catch(() => { if (current) setData({ active: [], history: [], recentClashes: [], counts: {}, earnings: { totalCreditedPaise: 0, awaitingInitialPaise: 0 } }); }).finally(() => { if (current) setLoading(false); }); return () => { current = false; }; }, [partnerId]);
  ...
  return <Section title={`Client Performance (${data.counts.total || 0})`}>{loading ? <Loader2 className="size-5 animate-spin text-[#DDAA42]" /> : <><div className="grid grid-cols-3 gap-2">{[["Pending",data.counts.pending || 0],["Approved",data.counts.approved || 0],["Successful",data.counts.successful || 0],["Rejected",data.counts.rejected || 0],["Expired",data.counts.expired || 0],["Clashes",data.counts.clashes || 0]].map(([label,value]) => <div key={label} className="rounded-lg bg-[#F8F7FA] p-2"><p className="text-[9px] font-bold uppercase text-[#8A8690]">{label}</p><p className="mt-1 text-lg font-bold text-[#121B35]">{value}</p></div>)}</div><div className="mt-2 rounded-xl bg-emerald-50 p-3 text-[11px] text-emerald-800"><strong>{formatMoney(data.earnings.totalCreditedPaise)}</strong> credited. <strong>{formatMoney(data.earnings.awaitingInitialPaise)}</strong> awaiting credit.</div>{data.active.length === 0 ? <p className="mt-3 text-[11.5px] text-[#68646F]">No active clients.</p> : <div className="mt-3 space-y-2">{data.active.slice(0,5).map((client) => <div key={client.id} className="rounded-xl border border-[#E4E0E7] p-3"><p className="text-[12px] font-bold text-[#121B35]">{client.clientName} | {client.mobileMasked}</p><p className="text-[10.5px] text-[#68646F]">{client.leadNumber} | {client.projectTitle}</p><p className="mt-1 text-[10px] font-bold capitalize text-[#8A681F]">{client.status}</p></div>)}</div>}{data.recentClashes.length > 0 && <details className="mt-3"><summary className="cursor-pointer text-[11px] font-bold text-[#68646F]">Recent clashes ({data.recentClashes.length})</summary><div className="mt-2 space-y-2">{data.recentClashes.map((clash) => <div key={clash.id} className="rounded-lg bg-amber-50 p-2 text-[10.5px] text-amber-900">{clash.clientName} | {clash.mobileMasked} | {clash.projectTitle} | {clash.direction}</div>)}</div></details>}{data.history.length > 0 && <details className="mt-3"><summary className="cursor-pointer text-[11px] font-bold text-[#68646F]">Rejected and expired history ({data.history.length})</summary><div className="mt-2 space-y-2">{data.history.map((client) => <div key={client.id} className="rounded-lg bg-[#F8F7FA] p-2 text-[10.5px] text-[#68646F]">{client.clientName} | {client.leadNumber} | {client.status}</div>)}</div></details>}</>}</Section>;
}

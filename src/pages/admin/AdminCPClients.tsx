...
type CPClient = {
  id: string; leadNumber: string; clientName: string; mobile: string; email: string; requirementType: "specific_project" | "general_requirement"; projectTitle: string; preferredLocation: string; propertyType: string; budget: string; status: Status; registeredAt: string; ownershipExpiresAt: string;
...
function ClientCard({ client, emailing, resendEmail, updateStatus }: { client: CPClient; emailing: boolean; resendEmail: (client: CPClient) => void; updateStatus: (client: CPClient, payload: Record<string, unknown>) => Promise<void> }) {
  ...
  return <article className="rounded-2xl border border-[#E4E0E7] bg-white p-4 md:p-5">
    <div className="grid gap-5 xl:grid-cols-[1.1fr_1fr_1fr_1fr_auto]">
      ...
      <div><p className="text-[10px] font-bold uppercase text-[#8A8690]">Property</p><p className="mt-1 text-sm font-bold text-[#121B35]">{client.projectTitle || "General requirement"}</p>{client.requirementType === "general_requirement" && (client.preferredLocation || client.propertyType) && <p className="mt-1 text-xs capitalize text-[#68646F]">{[client.preferredLocation, client.propertyType].filter(Boolean).join(" | ")}</p>}{client.budget && <p className="mt-1 text-xs text-[#68646F]">Budget: {client.budget}</p>}</div>

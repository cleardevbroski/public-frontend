...
import { fetchAdminCPClients } from "@/lib/api";
...
export default function AdminCPClients() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setClients(data.clients || []); setCounts(data.counts || {}); setPages(data.pagination?.pages || 1); setTotal(data.pagination?.total || 0);
  ...
  return <AdminLayout><div className="mb-6 flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-[28px] font-bold text-[#121B35]">CP Clients</h1><p className="mt-1 text-sm text-[#68646F]">Every client registered by a Channel Partner, including their selected project and ownership status.</p></div><div className="rounded-xl bg-[#FFF8E8] px-4 py-2 text-sm font-bold text-[#574619]">{total} client{total === 1 ? "" : "s"}</div></div>
    ...
    <div className="overflow-hidden rounded-2xl border border-[#E4E0E7] bg-white"><div className="overflow-x-auto"><table className="min-w-[1080px] w-full text-left"><thead className="border-b border-[#E4E0E7] bg-[#F8F7FA] text-[10px] font-bold uppercase tracking-wide text-[#68646F]"><tr><th className="px-5 py-4">Client</th><th className="px-5 py-4">Channel Partner</th><th className="px-5 py-4">Interested Project</th><th className="px-5 py-4">Registered / Expires</th><th className="px-5 py-4">Status</th></tr></thead><tbody className="divide-y divide-[#F0EDF2]">{loading ? <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-[#68646F]">Loading CP clients…</td></tr> : clients.length === 0 ? <tr><td colSpan={5} className="px-5 py-14 text-center"><UsersRound className="mx-auto size-9 text-[#D8D4DC]" /><p className="mt-3 text-sm text-[#68646F]">No CP clients found.</p></td></tr> : clients.map((client) => <tr key={client.id} className="align-top hover:bg-[#FCFCFD]"><td className="px-5 py-4"><p className="font-bold text-[#121B35]">{client.clientName}</p><p className="mt-1 text-xs text-[#68646F]">{client.mobile}{client.email ? ` · ${client.email}` : ""}</p><p className="mt-1 font-mono text-[10px] text-[#8A8690]">{client.leadNumber}</p></td><td className="px-5 py-4"><p className="font-bold text-[#121B35]">{client.channelPartner.name}</p><p className="mt-1 text-xs text-[#68646F]">{client.channelPartner.contactName} · {client.channelPartner.mobile}</p><p className="mt-1 text-[10px] text-[#8A8690]">{client.channelPartner.applicationNumber}{client.channelPartner.codeLast4 ? ` · code ••••${client.channelPartner.codeLast4}` : ""}</p></td><td className="px-5 py-4"><p className="font-semibold text-[#121B35]">{client.projectTitle}</p>{client.budget && <p className="mt-1 text-xs text-[#68646F]">Budget: {client.budget}</p>}</td><td className="px-5 py-4 text-xs text-[#3F3D46]"><p className="flex items-center gap-1.5"><CalendarDays className="size-3.5 text-[#DDAA42]" />{new Date(client.registeredAt).toLocaleDateString()}</p><p className="mt-1 text-[#68646F]">Expires: {new Date(client.ownershipExpiresAt).toLocaleDateString()}</p></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${client.status === "registered" ? "bg-emerald-50 text-emerald-700" : client.status === "expired" ? "bg-slate-100 text-slate-600" : "bg-amber-50 text-amber-800"}`}>{client.status}</span></td></tr>)}</tbody></table></div><div className="flex h-14 items-center justify-between border-t border-[#E4E0E7] px-5"><span className="text-xs text-[#68646F]">Page {page} of {pages}</span><div className="flex gap-2"><button disabled={page <= 1} onClick={() => setPage((value) => value - 1)} className="inline-flex size-8 items-center justify-center rounded-lg border border-[#E4E0E7] disabled:opacity-40"><ChevronLeft className="size-4" /></button><button disabled={page >= pages} onClick={() => setPage((value) => value + 1)} className="inline-flex size-8 items-center justify-center rounded-lg border border-[#E4E0E7] disabled:opacity-40"><ChevronRight className="size-4" /></button></div></div></div>
  </AdminLayout>;
}

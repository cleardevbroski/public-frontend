...
import { AlertCircle, CalendarClock, CheckCircle2, ChevronRight, Clock3, ExternalLink, FileSpreadsheet, FileText, LayoutList, Loader2, LogOut, MessageCircle, NotebookPen, PhoneCall, RefreshCw, Search, Send, UserRoundCheck, UsersRound, X } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { addCRMPartnerNote, crmEmployeeLogout, fetchMyCRMDashboard, fetchMyCRMPartner, fetchMyCRMPartners, hasCRMEmployeeSession, openCRMWhatsApp, saveCRMCallResult, saveCRMWhatsAppResult, startCRMCall } from "@/lib/api";
...
export default function CPManagement() {
  ...
  useEffect(() => { if (!hasCRMEmployeeSession()) navigate("/employee-login", { replace: true }); else void loadDashboard(); }, [navigate, loadDashboard]);
  ...
  const openPartner = async (partnerId: string) => {
    ...
    try {
      ...
      setCallPanel(false); setWhatsappPanel(false); setNote("");
    } catch (reason) { handleSessionError(reason); }
    finally { setBusy(""); }
  };
  ...
  const saveCall = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setNotice(callOutcome === "callback_requested" ? "Call saved and callback scheduled." : "Call result saved."); setCallPanel(false); setCallOutcome("no_answer"); setCallbackAt(""); setCallNote(""); await refreshSelected();
    } catch (reason) { handleSessionError(reason); }
    finally { setBusy(""); }
  };
  ...
  return <div className="min-h-[100dvh] bg-[#EFF1F4] text-[#3F3D46]">
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#0B1328]"><div className="mx-auto flex min-h-16 max-w-[1500px] flex-wrap items-center justify-between gap-2 px-4 py-2 md:px-6"><div className="flex items-center gap-3"><img src="/cleartitleone/logo.png" alt="ClearTitle One" className="size-9 rounded-full ring-2 ring-[#DDAA42]/60" /><div><p className="text-sm font-bold text-white">CP Management</p><p className="text-[10px] text-white/50">{employee ? `${employee.name} · ${employee.employeeId}` : "Employee workspace"}</p></div></div><div className="flex flex-wrap items-center justify-end gap-2"><button onClick={() => navigate("/cp-verification")} className="inline-flex h-9 items-center gap-2 rounded-md border border-white/15 px-3 text-[11px] font-bold text-white"><FileSpreadsheet className="size-4" />Imported CPs</button><button onClick={() => navigate("/broker-verification")} className="inline-flex h-9 items-center gap-2 rounded-md border border-white/15 px-3 text-[11px] font-bold text-white"><UsersRound className="size-4" />Brokers</button><button onClick={() => void Promise.all([loadDashboard(), loadPartners()])} className="grid size-9 place-items-center rounded-md border border-white/15 text-white" title="Refresh"><RefreshCw className={`size-4 ${busy === "partners" ? "animate-spin" : ""}`} /></button><button onClick={() => { crmEmployeeLogout(); navigate("/employee-login", { replace: true }); }} className="grid size-9 place-items-center rounded-md border border-white/15 text-white" title="Sign out"><LogOut className="size-4" /></button></div></div></header>
    <main className="mx-auto max-w-[1500px] space-y-5 px-4 py-6 md:px-6">
      <section className="flex flex-col gap-4 border-b border-[#D6D9DF] pb-5 md:flex-row md:items-end md:justify-between"><div><p className="text-[10px] font-bold uppercase text-[#805A0B]">Today’s work queue</p><h1 className="mt-1 text-[28px] font-bold text-[#121B35]">Channel Partner outreach</h1><p className="mt-1 text-xs text-[#68646F]">Open a partner, make the call, record the result, and complete due callbacks.</p></div><div className="min-w-[230px]"><div className="flex justify-between text-[10px] font-bold uppercase text-[#68646F]"><span>Assignment progress</span><span>{completion}%</span></div><div className="mt-2 h-2 overflow-hidden rounded bg-white"><div className="h-full bg-[#DDAA42]" style={{ width: `${completion}%` }} /></div></div></section>
      ...
      <section className="grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-[#D8DCE2] bg-[#D8DCE2] lg:grid-cols-6"><Metric label="Assigned" value={metrics.assigned} icon={UsersRound} /><Metric label="Pending" value={metrics.pending} icon={LayoutList} /><Metric label="Calls saved" value={metrics.callResults} icon={PhoneCall} /><Metric label="Due today" value={metrics.callbacksDueToday} icon={Clock3} /><Metric label="Overdue" value={metrics.callbacksOverdue} icon={CalendarClock} danger={metrics.callbacksOverdue > 0} /><Metric label="WhatsApp sent" value={metrics.whatsappSent} icon={Send} /></section>
      ...
      <div className="grid gap-3 rounded-lg border border-[#D8DCE2] bg-white p-4 md:grid-cols-[minmax(220px,1fr)_180px_160px]"><label className="relative"><Search className="absolute left-3 top-3 size-4 text-[#8A8690]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search assigned CPs..." className="h-10 w-full rounded-md border border-[#D8D3DA] pl-9 pr-3 text-xs" /></label><select value={stage} onChange={(event) => setStage(event.target.value)} className="h-10 rounded-md border border-[#D8D3DA] px-3 text-xs"><option value="">All stages</option>{stages.filter(Boolean).map((item) => <option key={item} value={item}>{stageLabels[item as CRMStage]}</option>)}</select><select value={due} onChange={(event) => setDue(event.target.value)} className="h-10 rounded-md border border-[#D8D3DA] px-3 text-xs"><option value="">All callbacks</option><option value="today">Due today</option><option value="overdue">Overdue</option></select></div>
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_520px]"><section className="overflow-hidden rounded-lg border border-[#D8DCE2] bg-white"><div className="divide-y divide-[#ECEEF1]">{busy === "partners" && !partners.length ? <div className="grid place-items-center py-16"><Loader2 className="size-6 animate-spin text-[#DDAA42]" /></div> : partners.map((profile) => <button key={profile.id} onClick={() => void openPartner(profile.partner.id)} className={`grid w-full items-center gap-3 px-4 py-4 text-left sm:grid-cols-[minmax(0,1fr)_140px_150px_28px] ${selected?.profile.partner.id === profile.partner.id ? "bg-[#FFF8E8]" : "hover:bg-[#FAFAFB]"}`}><div><strong className="block text-[13px] text-[#121B35]">{profile.partner.companyName}</strong><span className="mt-0.5 block text-[11px] text-[#68646F]">{profile.partner.contactName} · {profile.partner.mobile} · {profile.partner.city}</span></div><Stage value={profile.stage} /><div className="text-[10px] text-[#68646F]"><span className="block uppercase">Next callback</span><strong className={profile.nextFollowUpAt && new Date(profile.nextFollowUpAt) < new Date() ? "text-red-700" : "text-[#3F3D46]"}>{when(profile.nextFollowUpAt)}</strong></div><ChevronRight className="size-4 text-[#9A96A0]" /></button>)}{!partners.length && busy !== "partners" && <div className="py-16 text-center"><CheckCircle2 className="mx-auto size-8 text-emerald-600" /><p className="mt-2 text-sm font-bold text-[#121B35]">No CPs in this queue</p><p className="mt-1 text-xs text-[#68646F]">Change the filters or ask the administrator for an assignment.</p></div>}</div></section>
        <aside className="rounded-lg border border-[#D8DCE2] bg-white xl:sticky xl:top-20">{selected ? <PartnerWorkspace detail={selected} employee={employee} callPanel={callPanel} callOutcome={callOutcome} callbackAt={callbackAt} priority={priority} callNote={callNote} templateId={templateId} message={message} whatsappPanel={whatsappPanel} note={note} busy={busy} setCallOutcome={setCallOutcome} setCallbackAt={setCallbackAt} setPriority={setPriority} setCallNote={setCallNote} chooseTemplate={chooseTemplate} setMessage={setMessage} setNote={setNote} call={call} saveCall={saveCall} openWhatsApp={openWhatsApp} saveWhatsApp={saveWhatsApp} saveNote={saveNote} /> : <div className="py-20 text-center"><UserRoundCheck className="mx-auto size-9 text-[#C9C5CD]" /><p className="mt-3 text-xs text-[#68646F]">Select a Channel Partner to begin.</p></div>}</aside>
      </div>
    </main>
  </div>;
}

type WorkspaceProps = { detail: CRMPartnerDetail; employee: CRMEmployee | null; callPanel: boolean; callOutcome: CallOutcome; callbackAt: string; priority: string; callNote: string; templateId: string; message: string; whatsappPanel: boolean; note: string; busy: string; setCallOutcome: (value: CallOutcome) => void; setCallbackAt: (value: string) => void; setPriority: (value: string) => void; setCallNote: (value: string) => void; chooseTemplate: (value: string) => void; setMessage: (value: string) => void; setNote: (value: string) => void; call: () => Promise<void>; saveCall: (event: React.FormEvent) => Promise<void>; openWhatsApp: () => Promise<void>; saveWhatsApp: (outcome: "sent" | "not_sent" | "failed") => Promise<void>; saveNote: () => Promise<void> };

...
export default function CPManagement() {
  ...
  const rememberQueuePosition = useQueuePosition(selected?.profile.partner.id);
  ...
  return <div className="employee-workspace min-h-[100dvh] bg-[#EFF1F4] text-[#3F3D46] [&_input]:text-base [&_select]:text-base [&_textarea]:text-base sm:[&_input]:text-xs sm:[&_select]:text-xs sm:[&_textarea]:text-xs">
    <EmployeeHeader title="Registered CPs" employee={employee} filters={<div className="grid gap-2"><select aria-label="Stage" value={stage} onChange={(event) => setStage(event.target.value)} className="h-11 rounded-md border border-[#D8D3DA] px-3 sm:h-10"><option value="">All stages</option>{stages.filter(Boolean).map((item) => <option key={item} value={item}>{stageLabels[item as CRMStage]}</option>)}</select><select aria-label="Callbacks" value={due} onChange={(event) => setDue(event.target.value)} className="h-11 rounded-md border border-[#D8D3DA] px-3 sm:h-10"><option value="">All callbacks</option><option value="today">Due today</option><option value="overdue">Overdue</option></select></div>} />
    <main className="mx-auto max-w-[1500px] space-y-5 px-3 py-4 pb-28 sm:px-4 md:px-6 md:py-6 md:pb-6">
      ...
      <section className={`${selected ? "hidden xl:grid" : "grid"} grid-cols-3 gap-px overflow-hidden rounded-lg border border-[#D8DCE2] bg-[#D8DCE2] lg:grid-cols-6`}><Metric label="Assigned" value={metrics.assigned} icon={UsersRound} /><Metric label="Pending" value={metrics.pending} icon={LayoutList} /><Metric label="Calls saved" value={metrics.callResults} icon={PhoneCall} /><Metric label="Due today" value={metrics.callbacksDueToday} icon={Clock3} /><Metric label="Overdue" value={metrics.callbacksOverdue} icon={CalendarClock} danger={metrics.callbacksOverdue > 0} /><Metric label="WhatsApp sent" value={metrics.whatsappSent} icon={Send} /></section>
      ...
      <div className={`employee-search ${selected ? "hidden xl:block" : "block"}`}><label className="relative block"><Search className="absolute left-3 top-3.5 size-4 text-[#8A8690]" /><input type="search" aria-label="Search registered CPs" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, application no., phone or city" className="h-11 w-full rounded-md border border-[#BCA8D0] bg-white pl-9 pr-3" /></label></div>
...
function Metric({ label, value, icon: Icon, danger = false }: { label: string; value: number; icon: typeof UsersRound; danger?: boolean }) { return <div className="bg-white p-3"><div className="flex items-center justify-between"><span className="text-[9px] font-bold uppercase text-[#68646F]">{label}</span><Icon className={`size-4 ${danger ? "text-red-600" : "text-[#9A7427]"}`} /></div><strong className={`mt-2 block text-xl ${danger ? "text-red-700" : "text-[#121B35]"}`}>{value}</strong></div>; }

...
import { AlertCircle, ArrowLeft, CalendarClock, CheckCircle2, Clock3, LayoutList, Loader2, LogOut, MapPin, MessageCircle, PhoneCall, RefreshCw, Search, Send, UserRoundCheck, X } from "lucide-react";
import { useNavigate } from "react-router-dom";
import EmployeeWorkspaceNav from "@/components/crm/EmployeeWorkspaceNav";
import { crmEmployeeLogout, fetchCRMEmployeeMe, fetchMyCPProspect, fetchMyCPProspects, hasCRMEmployeeSession, openCPProspectWhatsApp, saveBrokerCallResult, saveCPProspectWhatsAppResult, startCPProspectCall } from "@/lib/api";
...
export default function BrokerVerification() {
  ...
  const [filters, setFilters] = useState({ search: "", status: "", area: "", propertyType: "", due: "" });
  ...
  const loadQueue = useCallback(async () => {
    setBusy("queue");
    try {
      const data = await fetchMyCPProspects({ prospectType: "broker", ...filters, limit: 100 });
  ...
  const open = async (id: string) => {
    ...
    try {
      ...
      setTemplateId(first?.id || ""); setMessage(first ? renderMessage(first, detail.prospect, employee) : "");
  ...
  const openWhatsApp = async () => {
    if (!selected || !templateId || !message) return;
  ...
  const whatsappDisabled = selected?.prospect.verificationStatus === "wrong_number";
  ...
  return <div className="min-h-[100dvh] bg-[#EFF1F4] text-[#3F3D46] [&_input]:text-base [&_select]:text-base [&_textarea]:text-base sm:[&_input]:text-xs sm:[&_select]:text-xs sm:[&_textarea]:text-xs">
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#0B1328]"><div className="mx-auto flex min-h-16 max-w-[1500px] items-center justify-between gap-2 px-3 py-2 sm:px-4 md:px-6"><div className="flex min-w-0 items-center gap-3"><img src="/cleartitleone/logo.png" alt="ClearTitle One" className="size-9 shrink-0 rounded-full ring-2 ring-[#DDAA42]/60" /><div className="min-w-0"><p className="truncate text-sm font-bold text-white">Broker Calls</p><p className="truncate text-[10px] text-white/50">{employee ? `${employee.name} · ${employee.employeeId}` : "Employee workspace"}</p></div></div><div className="flex shrink-0 items-center gap-2"><button onClick={() => void loadQueue()} className="grid size-11 place-items-center rounded-md border border-white/15 text-white md:size-9" title="Refresh" aria-label="Refresh broker queue"><RefreshCw className={`size-4 ${busy === "queue" ? "animate-spin" : ""}`} /></button><button onClick={() => { crmEmployeeLogout(); navigate("/employee-login", { replace: true }); }} className="grid size-11 place-items-center rounded-md border border-white/15 text-white md:size-9" title="Sign out" aria-label="Sign out"><LogOut className="size-4" /></button></div></div></header>
    <main className="mx-auto max-w-[1500px] space-y-5 px-3 py-4 pb-28 sm:px-4 md:px-6 md:py-6 md:pb-6">
      <section className={`${selected ? "hidden xl:block" : "block"} border-b border-[#D6D9DF] pb-5`}><p className="text-[10px] font-bold uppercase text-[#805A0B]">Assigned broker queue</p><h1 className="mt-1 text-2xl font-bold text-[#121B35] sm:text-[28px]">Call and qualify brokers</h1><p className="mt-1 text-xs text-[#68646F]">Record the call result, project interest, follow-up, and market areas.</p></section>
      {error && <Notice tone="error" text={error} dismiss={() => setError("")} />}{notice && <Notice tone="success" text={notice} dismiss={() => setNotice("")} />}
      <section className={`${selected ? "hidden xl:grid" : "grid"} grid-cols-2 gap-px overflow-hidden rounded-lg border border-[#D8DCE2] bg-[#D8DCE2] lg:grid-cols-6`}><Metric label="Assigned" value={metrics.total} icon={LayoutList} /><Metric label="Pending" value={metrics.pending} icon={Clock3} /><Metric label="Interested" value={metrics.interested} icon={UserRoundCheck} /><Metric label="Not interested" value={metrics.notInterested} icon={CheckCircle2} /><Metric label="Callbacks" value={metrics.callback} icon={CalendarClock} /><Metric label="WhatsApp sent" value={metrics.whatsappSent} icon={Send} /></section>
      <section className={`${selected ? "hidden xl:grid" : "grid"} gap-3 rounded-lg border border-[#D8DCE2] bg-white p-3 sm:p-4 md:grid-cols-3 xl:grid-cols-6`}><label className="relative md:col-span-2"><Search className="absolute left-3 top-3.5 size-4 text-[#8A8690]" /><input value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} placeholder="Search broker, phone, area..." className="h-11 w-full rounded-md border border-[#D8D3DA] pl-9 pr-3 sm:h-10" /></label><Filter value={filters.status} onChange={(value) => setFilters({ ...filters, status: value })} label="All results" options={[["pending", "Pending"], ["active", "Interested"], ["inactive", "Not interested"], ["callback_requested", "Call again"], ["no_answer", "Not picked"], ["busy", "Busy"], ["wrong_number", "Wrong number"]]} /><Filter value={filters.area} onChange={(value) => setFilters({ ...filters, area: value })} label="All areas" options={areasInQueue.map((item) => [item, item])} /><Filter value={filters.propertyType} onChange={(value) => setFilters({ ...filters, propertyType: value })} label="All property types" options={propertyTypes.map((item) => [item, title(item)])} /><Filter value={filters.due} onChange={(value) => setFilters({ ...filters, due: value })} label="All callbacks" options={[["overdue", "Overdue"]]} /></section>
      <div className="grid items-start gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
        ...
        {selected ? <section className="min-w-0 space-y-5"><button onClick={() => setSelected(null)} className="flex min-h-11 w-full items-center gap-2 rounded-md border border-[#D8DCE2] bg-white px-4 text-xs font-bold text-[#121B35] xl:hidden"><ArrowLeft className="size-4" />Back to assigned brokers</button>
          ...
          <section className="rounded-lg border border-[#E4E0E7] bg-white p-4 sm:p-5"><div className="flex items-center gap-2"><MessageCircle className="size-4 text-[#168B52]" /><h3 className="text-sm font-bold text-[#121B35]">WhatsApp project message</h3></div>{selected.templates.length ? <><label className="mt-4 block"><span className="text-[10px] font-bold uppercase text-[#68646F]">Admin template</span><select value={templateId} onChange={(event) => chooseTemplate(event.target.value)} className="mt-1 h-11 w-full rounded-md border border-[#D8D3DA] px-3"><option value="">Choose template</option>{selected.templates.map((template) => <option key={template.id} value={template.id}>{template.name} · {template.kind.replace("_", " ")}</option>)}</select></label><label className="mt-3 block"><span className="text-[10px] font-bold uppercase text-[#68646F]">Message preview</span><textarea readOnly value={message} className="mt-1 h-40 w-full resize-y rounded-md border border-[#D8D3DA] bg-[#F8F7FA] p-3 leading-5" /></label><button onClick={() => void openWhatsApp()} disabled={busy === "whatsapp" || !templateId || !message || whatsappDisabled} className="mt-3 inline-flex h-11 w-full items-center justify-center gap-2 rounded-md bg-[#168B52] text-xs font-bold text-white disabled:opacity-40">{busy === "whatsapp" ? <Loader2 className="size-4 animate-spin" /> : <MessageCircle className="size-4" />}Open WhatsApp</button>{whatsappDisabled && <p className="mt-2 text-[10px] text-red-700">WhatsApp is disabled because this contact is marked as a wrong number.</p>}</> : <p className="mt-3 rounded-md bg-[#F8F7FA] p-3 text-xs text-[#68646F]">No broker message template is active. Ask the administrator to create one for Brokers or All CRM contacts.</p>}{whatsappPanel && <div className="mt-3 rounded-md border border-emerald-200 bg-emerald-50 p-3"><p className="text-xs font-bold text-emerald-900">Did you press Send in WhatsApp?</p><div className="mt-2 grid grid-cols-2 gap-2"><button onClick={() => void saveWhatsApp("sent")} disabled={busy === "whatsapp-result"} className="h-10 rounded-md bg-emerald-700 text-[11px] font-bold text-white">Sent</button><button onClick={() => void saveWhatsApp("not_sent")} disabled={busy === "whatsapp-result"} className="h-10 rounded-md bg-white text-[11px] font-bold text-[#3F3D46]">Not sent</button></div></div>}</section>
...
function BrokerQueue({ prospects, selectedId, busy, onOpen }: { prospects: CPProspect[]; selectedId?: string; busy: string; onOpen: (id: string) => Promise<void> }) {
  return <section className="overflow-hidden rounded-lg border border-[#E4E0E7] bg-white xl:sticky xl:top-20"><div className="border-b border-[#E4E0E7] px-4 py-3"><h2 className="text-xs font-bold text-[#121B35]">Assigned brokers</h2></div><div className="max-h-[calc(100dvh-190px)] divide-y divide-[#F0EDF1] overflow-y-auto">{prospects.map((item) => <button key={item.id} onClick={() => void onOpen(item.id)} className={`block w-full p-4 text-left ${selectedId === item.id ? "bg-[#FFF8E8]" : "hover:bg-[#FAF9FA]"}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><strong className="block truncate text-[12px] text-[#121B35]">{item.contact.name || item.company.name || "Unnamed broker"}</strong><span className="mt-1 block text-[10px] text-[#68646F]">{item.contact.mobile}</span></div>{busy === item.id ? <Loader2 className="size-4 animate-spin" /> : <BrokerStatus item={item} compact />}</div><p className="mt-2 flex items-center gap-1 text-[10px] text-[#68646F]"><MapPin className="size-3" />{item.business.areasOfOperation[0] || item.address.city || "Area not collected"}</p>{item.nextFollowUpAt && <p className="mt-1 text-[9px] font-bold text-amber-800">Follow-up: {when(item.nextFollowUpAt)}</p>}</button>)}{!prospects.length && <p className="p-10 text-center text-xs text-[#68646F]">No assigned brokers match these filters.</p>}</div></section>;
}

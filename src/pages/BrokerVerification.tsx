...
export default function BrokerVerification() {
  ...
  return <div className="min-h-[100dvh] bg-[#EFF1F4] text-[#3F3D46] [&_input]:text-base [&_select]:text-base [&_textarea]:text-base sm:[&_input]:text-xs sm:[&_select]:text-xs sm:[&_textarea]:text-xs">
    ...
    <main className="mx-auto max-w-[1500px] space-y-3 px-3 py-3 pb-28 sm:px-4 md:px-6 md:pb-6">
      ...
      <div className={`${selected ? "hidden xl:block" : "sticky"} top-[4.1rem] z-30 border-b border-[#7447A8] bg-[#EFF1F4] py-2`}><label className="relative block"><Search className="absolute left-3 top-3.5 size-4 text-[#8A8690]" /><input value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} placeholder="Search assigned brokers..." className="h-11 w-full rounded-md border border-[#BCA8D0] bg-white pl-9 pr-3 shadow-sm" /></label></div>

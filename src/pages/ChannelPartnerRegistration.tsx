...
export default function ChannelPartnerRegistration() {
  ...
  const [mobileTab, setMobileTab] = useState<"register" | "clients">("register");
  ...
  return <div className="partner-page min-h-[100dvh] bg-[#EFF1F4] text-[#3F3D46]">
    <PartnerNavigation active="clients" onLogout={partner ? () => { clearChannelPartnerSession(); setPartner(null); setClients([]); setProjects([]); setMobileTab("register"); } : undefined} />
    <main className="mx-auto max-w-[1100px] px-4 py-10">
      <section className="overflow-hidden rounded-3xl border border-[#E4E0E7] bg-white shadow-xl shadow-[#121B35]/5"><div className="partner-form-intro border-b border-[#E4E0E7] bg-[#FFFDF7] p-6 md:p-8"><div className="flex gap-3"><span className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-[#FFF1C8]"><KeyRound className="size-6 text-[#9A6A13]" /></span><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#B88422]">One-page registration</p><h1 className="mt-1 text-2xl font-bold text-[#121B35]">Register your clients</h1><p className="mt-1 text-sm text-[#68646F]">{partner ? "Your authenticated Channel Partner account is ready." : "Enter your code, see your partner details, and add as many clients as needed."}</p></div></div>{!partner && <form onSubmit={verify} className="mt-6 grid gap-3 md:grid-cols-[minmax(0,1fr)_160px]"><input value={code} onChange={(event) => updateCode(event.target.value)} placeholder="Enter Channel Partner Code, e.g. CT-0001" className={`${inputClass} font-bold tracking-[.12em]`} required /><button disabled={busy || !code.trim()} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-[#0B1328] px-5 text-sm font-bold text-white disabled:opacity-50">{busy ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />}Show partner details</button></form>}{error && <p role="alert" className="mt-4 rounded-xl bg-red-50 p-3 text-xs text-red-700">{error}</p>}{partner && <details className="mt-4"><summary className="cursor-pointer py-2 text-sm font-bold text-emerald-800">{partner.name} <span className="text-xs font-normal">({partner.code})</span></summary><div className="mt-3 grid gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 sm:grid-cols-2 lg:grid-cols-4"><Detail label="Partner" value={partner.name} /><Detail label="Contact" value={partner.contactName} /><Detail label="Mobile" value={partner.mobile} /><Detail label="Email" value={partner.email} /><Detail label="Location" value={`${partner.city}, ${partner.state}`} /><Detail label="Partner type" value={partner.type === "individual" ? "Individual Partner" : "Company / Firm"} /><Detail label="Partner code" value={partner.code} /></div><div className="mt-3 flex flex-wrap gap-2"><a href="/cp-dashboard" className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#0B1328] px-4 text-xs font-bold text-white"><LayoutDashboard className="size-4" />View Dashboard</a><button onClick={() => { clearChannelPartnerSession(); setPartner(null); setClients([]); setProjects([]); }} className="inline-flex h-10 items-center rounded-xl border border-[#D8D4DC] px-4 text-xs font-bold text-[#68646F]">Use another CP code</button></div></details>}</div>
        <div className="flex gap-2 border-b border-[#E4E0E7] p-3 lg:hidden" role="tablist" aria-label="Client workspace">
          <button type="button" role="tab" aria-selected={mobileTab === "register"} aria-controls="client-registration-form" onClick={() => setMobileTab("register")} className={`min-h-11 flex-1 rounded-md px-3 text-sm font-semibold ${mobileTab === "register" ? "bg-[#121B35] text-white" : "bg-[#F3F4F6]"}`}>Register client</button>
          <button type="button" role="tab" aria-selected={mobileTab === "clients"} aria-controls="active-client-list" onClick={() => setMobileTab("clients")} className={`min-h-11 flex-1 rounded-md px-3 text-sm font-semibold ${mobileTab === "clients" ? "bg-[#121B35] text-white" : "bg-[#F3F4F6]"}`}>My clients ({clients.length})</button>
        </div>
        <div className="partner-form-content grid items-start gap-6 p-6 md:p-8 lg:grid-cols-[minmax(0,1fr)_360px]">
          <form id="client-registration-form" onSubmit={submit} className={`${mobileTab === "register" ? "grid" : "hidden lg:grid"} gap-5 md:grid-cols-2`}>
          ...
          <aside id="active-client-list" className={`${mobileTab === "clients" ? "block" : "hidden lg:block"} rounded-lg border border-[#E4E0E7] bg-[#FCFCFD] p-4`}>

    ...
    <main className="mx-auto max-w-[1280px] px-4 py-8 md:px-6 md:py-10">
      {loading ? <DashboardSkeleton /> : error ? <ErrorState error={error} retry={load} /> : dashboard ? <>
        ...
        <section className="flex flex-col gap-5 border-b border-[#D9DCE2] pb-7 md:flex-row md:items-end md:justify-between">
          <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#805A0B]">Partner workspace</p><h1 className="display-heading mt-2 text-4xl text-[#121B35] md:text-5xl">Welcome, {dashboard.partner.name}</h1><p className="mt-2 text-sm text-[#68646F]">Track every client from registration through settlement.</p></div>

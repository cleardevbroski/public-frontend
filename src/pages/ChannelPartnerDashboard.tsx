...
export default function ChannelPartnerDashboard() {
  ...
  return <div className="partner-page min-h-[100dvh] bg-[#EFF1F4] text-[#3F3D46]">
    ...
    <main className="mx-auto max-w-[1280px] px-4 py-8 md:px-6 md:py-10">
      {loading ? <DashboardSkeleton /> : error ? <ErrorState error={error} retry={load} /> : dashboard ? <>
        ...
        <section className="partner-welcome flex flex-col gap-5 border-b border-[#D9DCE2] pb-7 md:flex-row md:items-end md:justify-between">
          <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#805A0B]">Partner workspace</p><h1 className="mt-2 text-2xl font-bold text-[#121B35] md:text-3xl">Welcome, {dashboard.partner.name}</h1><p className="mt-2 text-sm text-[#68646F]">Track every client from registration through settlement.</p></div>
          <div className="flex flex-wrap gap-2"><a href="/cp-registration" className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#DDAA42] px-4 text-sm font-bold text-[#0B1328] active:translate-y-px"><UserPlus className="size-4" />Register Client</a></div>
        </section>

        <section aria-label="Client summary" className="mt-4 grid grid-cols-3 gap-2 md:grid-cols-4 xl:grid-cols-7">
        ...
        <section className="mt-6 overflow-hidden rounded-2xl bg-[#123D2D] text-white shadow-[0_18px_45px_rgba(13,62,44,.18)]">
          <div className="partner-earnings grid gap-7 p-6 md:grid-cols-[1.25fr_1fr] md:p-8">
...
function CPHeader({ onLogout }: { onLogout?: () => void }) {
  return <PartnerNavigation active="dashboard" onLogout={onLogout} />;
}

function Metric({ label, value, icon: Icon }: { label: string; value: number; icon: React.ComponentType<{ className?: string }> }) {
  return <article className="partner-summary rounded-lg border border-[#D9DCE2] bg-white p-4"><div className="flex items-center justify-between"><span className="text-xs font-bold text-[#68646F]">{label}</span><Icon className="size-4 text-[#9A6A13]" /></div><p className="mt-3 text-3xl font-bold text-[#121B35]">{value}</p></article>;
}

...
export default function BulkPropertyMediaImporter({
  ...
  const addFiles = (files: FileList | File[]) => {
    ...
    const imageFiles = source.filter((file) => imageTypes.has(file.type) && file.size <= 5 * 1024 * 1024);
    ...
    const prepared = source.map((file, index): MediaItem => {
      if (imageTypes.has(file.type)) {
        if (file.size > 5 * 1024 * 1024) return { id: mediaId(file, index), file, destination: "gallery", status: "error", message: "Images must be 5 MB or smaller." };
      ...
      if (file.type === "video/mp4") {
        if (file.size > 15 * 1024 * 1024) return { id: mediaId(file, index), file, destination: "walkthrough", status: "error", message: "Walkthrough videos must be 15 MB or smaller." };

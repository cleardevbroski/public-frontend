...
type Props = {
  ...
  updateDetail: (configuration: string, updates: Partial<VillaConfigurationDetail>) => void;
...
export default function VillaDetailsFields(props: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <label className="block text-[13px] font-semibold text-[#243559] mb-2">Villa Type <span className="text-[#E8C66A]">*</span></label>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input value={props.configInput} onChange={(event) => props.setConfigInput(event.target.value)} onKeyDown={(event) => event.key === "Enter" && (event.preventDefault(), props.addConfig())} placeholder="e.g. 3 BHK or 4 BHK" className="flex-1 px-4 py-3 border border-[#D5DEF2] rounded-xl text-[14px] focus:outline-none focus:border-[#C9A24E]" />
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {props.details.configurationDetails.map((row) => (
      ...
      {props.details.configurationDetails.length > 0 && (
        <div>
          <h3 className="text-[14px] font-bold text-[#1E3A8A] mb-2">Per-configuration Villa details</h3>
                    ...
                    <tr key={row.configuration} className="border-t border-[#E2E9FB] align-top">
                      ...
                      {(["price", "plotArea", "builtUpArea", "superArea"] as const).map((key) => (
                        <td key={key} className="px-2 py-2">
                          <input className={inputClass} value={row[key]} placeholder={key === "price" ? "₹2.80 Cr" : "2400 sqft"} onChange={(event) => props.updateDetail(row.configuration, { [key]: event.target.value })} />
                          {field(key) && <p className="text-[10px] text-red-600 mt-1">{field(key)}</p>}
                        </td>
                      ))}
                      {(["bedrooms", "bathrooms"] as const).map((key) => (
                        <td key={key} className="px-2 py-2">
                          <input type="number" min={1} step={1} className={`${inputClass} min-w-[72px]`} value={row[key]} onChange={(event) => props.updateDetail(row.configuration, { [key]: Number(event.target.value) })} />
                          {field(key) && <p className="text-[10px] text-red-600 mt-1">{field(key)}</p>}
                        </td>
                      ))}
                    </tr>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 rounded-2xl border border-[#D5DEF2]/50 bg-[#F1F5FF]/40 p-5">
        <div>
          <label className="block text-[13px] font-semibold text-[#243559] mb-2">Plot Dimensions</label>
        ...
        <div>
          <label className="block text-[13px] font-semibold text-[#243559] mb-2">Number of Floors</label>
        ...
        <div>
          <label className="block text-[13px] font-semibold text-[#243559] mb-2">Plot Facing <span className="text-[#E8C66A]">*</span></label>
        ...
        <div>
          <label className="block text-[13px] font-semibold text-[#243559] mb-2">Road Width Facing</label>
        ...
        {props.details.privateGarden && <div>
          <label className="block text-[13px] font-semibold text-[#243559] mb-2">Private Garden Area <span className="text-[#E8C66A]">*</span></label>
        ...
        {props.details.terrace && <div>
          <label className="block text-[13px] font-semibold text-[#243559] mb-2">Terrace Details</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-[13px] font-semibold text-[#243559] mb-2">{underConstruction ? "Expected Completion" : "Ready Since"} <span className="text-[#E8C66A]">*</span></label>

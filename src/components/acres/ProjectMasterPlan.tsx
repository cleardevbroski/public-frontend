...
export default function ProjectMasterPlan({ property }: { property: Property }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-[#DDE2EA] bg-white p-5 shadow-sm md:p-7" aria-labelledby="master-plan-heading">
    <h2 id="master-plan-heading" className="text-[22px] font-extrabold text-[#172039]">{property.masterPlan?.title || `${property.title} Master Plan`}</h2>
    <div className="mt-5 grid gap-6 md:grid-cols-[minmax(280px,0.9fr)_minmax(0,1.1fr)]">
      {(image || legacyPdf) && <div className="flex min-h-64 items-center justify-center overflow-hidden rounded-xl border border-[#E5E8EE] bg-[#F4F5F7]">{image ? <img src={image} alt={`${property.title} master plan`} loading="lazy" className="max-h-[440px] w-full object-contain" /> : <a href={legacyPdf} target="_blank" rel="noreferrer" className="rounded-lg bg-[#172039] px-5 py-3 text-[13px] font-bold text-white">View Master Plan PDF</a>}</div>}
      <div className={!expanded ? "max-h-[290px] overflow-hidden" : ""}>

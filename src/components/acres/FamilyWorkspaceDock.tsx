...
import { ArrowRight, Check, MessagesSquare, UsersRound, Vote, X } from "lucide-react";
...
export default function FamilyWorkspaceDock() {
  ...
  useEffect(() => {
    const selector = "[data-public-bottom-bar]";
    let bars: Element[] = [];
    const measure = () => {
      const height = Math.max(0, ...bars.map((bar) => bar.getBoundingClientRect().height));
      document.documentElement.style.setProperty("--public-bottom-bar-height", `${Math.ceil(height)}px`);
    };
    const resize = typeof ResizeObserver === "undefined" ? null : new ResizeObserver(measure);
    const discover = () => {
      resize?.disconnect();
      bars = Array.from(document.querySelectorAll(selector));
      bars.forEach((bar) => resize?.observe(bar));
      measure();
    };
    // The route can mount its action bar after this shared control has mounted.
    const mutations = new MutationObserver((records) => {
      if (records.some((record) => [...record.addedNodes, ...record.removedNodes].some((node) =>
        node instanceof Element && (node.matches(selector) || node.querySelector(selector))))) discover();
    });
    ...
    return () => {
      ...
      document.documentElement.style.removeProperty("--public-bottom-bar-height");
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const outside = (event: PointerEvent) => { if (!dockRef.current?.contains(event.target as Node)) setOpen(false); };
  ...
  return <aside ref={dockRef} className="public-family-dock fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2 lg:bottom-5 lg:right-5" aria-label="Family home decision workspace">
    {open && <div id={panelId} role="region" aria-label="Decide together details" className="w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-[22px] border border-[#D9D1C4] bg-[#FBF9F4] shadow-[0_22px_60px_rgba(11,19,40,.22)]">
      <div className="relative overflow-hidden bg-[#172039] px-5 pb-5 pt-4 text-white"><div className="absolute -right-9 -top-9 size-28 rounded-full border border-[#E2B757]/25" /><div className="relative flex items-start justify-between gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#DDAA42] text-[#172039]"><UsersRound className="size-5" /></span><button type="button" onClick={close} aria-label="Close family workspace explanation" className="grid size-11 place-items-center rounded-lg text-white/60 transition hover:bg-white/10 hover:text-white"><X className="size-4" /></button></div><h2 className="relative mt-4 text-[20px] font-extrabold tracking-[-.03em]">Decide on a home together</h2><p className="relative mt-1 max-w-[34ch] text-[11px] leading-5 text-white/70">Keep the family&apos;s property discussion, votes and questions together instead of losing them across different chats.</p></div>
    ...
    <button ref={triggerRef} type="button" onClick={() => setOpen((value) => !value)} aria-label={workspace ? "Family workspace" : "Decide together"} title={workspace ? "Family workspace" : "Decide together"} aria-controls={panelId} aria-expanded={open} className="family-dock-trigger group inline-flex h-13 items-center gap-2.5 rounded-2xl border border-[#DDAA42]/60 bg-[#172039] px-3.5 text-white shadow-[0_14px_38px_rgba(11,19,40,.28)] transition duration-200 hover:-translate-y-0.5 hover:bg-[#202B48] active:translate-y-0 active:scale-[.98] focus:outline-none focus:ring-2 focus:ring-[#DDAA42] focus:ring-offset-2"><span className="relative grid size-7 place-items-center rounded-lg bg-[#DDAA42] text-[#172039]"><UsersRound className="size-4" />{workspace && <span className="absolute -right-1 -top-1 size-2.5 rounded-full border-2 border-[#172039] bg-emerald-400" />}</span><span className="family-dock-label text-left"><span className="block text-[11px] font-extrabold leading-4">{workspace ? "Family workspace" : "Decide together"}</span><span className="hidden text-[9px] leading-3 text-white/60 sm:block">Compare · vote · chat</span></span></button>
  </aside>;
}

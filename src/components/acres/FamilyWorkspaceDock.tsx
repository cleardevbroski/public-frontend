...
const INTRO_SEEN_KEY = "cleartitle_family_dock_seen";
...
export default function FamilyWorkspaceDock() {
  ...
  useEffect(() => {
    ...
    sync();
    const showIntroduction = !localStorage.getItem(INTRO_SEEN_KEY);
    if (showIntroduction) {
      setOpen(true);
  ...
  return <aside className="fixed bottom-44 right-4 z-50 flex flex-col items-end gap-2 lg:bottom-5 lg:right-5" aria-label="Family home decision workspace">

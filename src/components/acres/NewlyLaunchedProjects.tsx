      ...
      <div className="max-w-[1200px] mx-auto px-5">
        <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <span className="size-12 rounded-xl bg-[#1E3A8A] flex items-center justify-center text-[#E8C66A]">
              <Building2 className="size-6" />
            </span>
            <div>
              <h2 className="text-[26px] md:text-[32px] font-bold text-[#1E3A8A]">
                Newly launched <span className="text-gold-gradient">projects</span>
              </h2>
              <p className="text-[13px] text-[#6E7488]">Less upfront payment</p>
            </div>
          </div>
        ...
        <div ref={scrollerRef} className="flex gap-5 overflow-x-auto no-scrollbar pb-3 scroll-smooth">
          {newlyLaunchedProjects.map((p) => (
            <div key={p.id} className="shrink-0 w-[460px] max-w-[88vw] bg-white shadow-sm border border-[#D5DEF2]/50 overflow-hidden">
              <div className="p-5">
                ...
                <div className="flex gap-4 mt-3">
                  <div className="relative shrink-0">
                    ...
                    {p.rera && (
                      <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 inline-flex items-center gap-0.5 bg-[#1E3A8A] text-white text-[9px] font-bold px-1.5 py-0.5 rounded">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[14px] font-bold text-[#1E3A8A] mt-1">
                      {p.price} <span className="text-[12px] font-medium text-[#6E7488]">| {p.config}</span>
                    </p>
                    <p className="text-[12px] font-semibold text-[#1E7A46] mt-1 truncate">{p.priceTrend}</p>
                  </div>
                </div>
              </div>
              <div className="flex items-center justify-between gap-3 border-t border-dashed border-[#D5DEF2] px-5 py-3 bg-[#FAFCFF]">
                <span className="flex items-center gap-1.5 text-[12px] text-[#6E7488]">
                  <Tag className="size-3.5 text-[#D4AF37]" /> Preferred options @zero brokerage
                </span>
